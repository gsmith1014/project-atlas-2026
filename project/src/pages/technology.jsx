/* global React, Eyebrow, Section, SectionHeader, ImgPh, Btn, Stat, NavA, EcgLine, navTo */
...
function TechnologyPage() {
  return (
    <div className="page-fade" data-screen-label="02 Technology" data-page="technology">
      ...
      {/* Phase-space tomography illustration */}
      <Section dark>
      ...
      {/* Improved workflow + cost reduction story */}
      <Section>
      ...
      <Section dark>
        <div className="row row-2" style={{ gridTemplateColumns: '1fr 1.3fr', gap: 80, alignItems: 'start' }}>
          ...
          <div className="row row-2" style={{ gap: 0 }}>
            {[
              { label: 'PH total cost of care', value: '\u221244', unit: '%' },
              { label: 'CAD diagnostic pathway', value: '\u221232', unit: '%' },
              { label: 'Saved per case', value: '~$1,200', unit: '' },
              { label: 'Scan to report', value: '<8', unit: 'min' },
            ].map((m, i) => (
              <div key={i} style={{ padding: '24px 28px', borderTop: i < 2 ? 0 : '1px solid #1F2A3D', borderLeft: i % 2 === 1 ? '1px solid #1F2A3D' : 0 }}>
                <div style={{ fontFamily: 'var(--f-mono)', fontSize: 12, color: '#98A2B3', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{m.label}</div>
                <div style={{ fontFamily: 'var(--f-sans)', fontSize: 'clamp(48px, 6vw, 80px)', fontWeight: 500, color: '#F4F6F9', letterSpacing: '-0.02em', marginTop: 12, lineHeight: 1 }}>
                  {m.value}{m.unit && <span style={{ fontSize: '0.4em', verticalAlign: '0.4em', marginLeft: 4, color: '#98A2B3' }}>{m.unit}</span>}
                </div>
              </div>
            ))}
          </div>
        </div>
        <div className="meta" style={{ color: '#98A2B3', marginTop: 48 }}>
          Source: CorVista Health analysis of total cost of care and diagnostic pathway modeling, 2025.
        </div>
      </Section>
      ...
      <Section>
        <div style={{ borderTop: '1px solid var(--rule)', paddingTop: 56 }}>
          <div className="row row-2" style={{ gridTemplateColumns: '1.2fr 1fr', gap: 80, alignItems: 'center' }}>
            <ImgPh label="System components — sensor, tablet, cloud, report" ratio="4/3" />
            <div>
              <Eyebrow>The system</Eyebrow>
              <h3 style={{ marginTop: 18, marginBottom: 24 }}>Three components. One workflow.</h3>
              <ul className="bullets">
                <li><strong>CorVista Sensor</strong> — single-use, hospital-grade wearable that records cardiovascular biosignals at rest.</li>
                <li><strong>CorVista Acquisition Device</strong> — tablet-based device that drives capture and uploads to the cloud over LTE or Wi-Fi.</li>
                <li><strong>CorVista Cloud</strong> — HIPAA-compliant analytics platform that runs the CPST pipeline and routes the report.</li>
              </ul>
              <div style={{ marginTop: 36 }}>
                <NavA to="evidence" className="btn btn-ghost">Read the clinical evidence<span className="arrow">→</span></NavA>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Regulatory footer */}
      <Section dark>
        <div className="row row-2" style={{ gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'end' }}>
          <h2 style={{ color: '#F4F6F9' }}>Cleared. Validated. Deployed.</h2>
          <div className="row row-3" style={{ gap: 24 }}>
            <Stat label="FDA Class II" value="510(k)" desc="Cleared for marketing in the United States." />
            <Stat label="Geographies" value="3" desc="US · EU · Canada — and expanding." />
            <Stat label="Publications" value="50+" desc="Peer-reviewed studies, abstracts, and posters." />
          </div>
        </div>
      </Section>
    </div>
