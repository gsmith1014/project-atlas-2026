...
function EcgLine({ color = "var(--blue)", height = 60, segments = 6, strokeWidth = 1.5 }) {
  ...
  for (let i = 0; i < segments; i++) {
    const x0 = i * segW;
    d += ` L ${x0 + segW*0.18} ${baseline}`;
    d += ` L ${x0 + segW*0.22} ${baseline - height*0.08}`;
    d += ` L ${x0 + segW*0.26} ${baseline}`;
    d += ` L ${x0 + segW*0.30} ${baseline + height*0.05}`;
    d += ` L ${x0 + segW*0.34} ${baseline - height*0.45}`;
    d += ` L ${x0 + segW*0.38} ${baseline + height*0.20}`;
    d += ` L ${x0 + segW*0.50} ${baseline}`;
    d += ` L ${x0 + segW*0.60} ${baseline - height*0.12}`;
...
export function navTo(slug) {
  window.dispatchEvent(new CustomEvent('cv-navigate', { detail: slug }));
}

export function NavA({ to, className = "", children, ...rest }) {
  return (
    <a href={`#${to}`} className={className} onClick={(e) => { e.preventDefault(); navTo(to); }} {...rest}>{children}</a>
  );
}

export { BrandMark, Brand, EcgLine, ImgPh, Btn, Stat, Eyebrow, Section, SectionHeader, Marquee, Tabs };
